...
void client_encrypted_session<SocketType>::handle_read_encrypted_response(
    ...
    if (protocol::get_message_type(response_message) ==
        ...
        spdlog::debug("file list request got denied by host {}.",
    ...
    boost::asio::async_connect(
        ...
        [this, me = client_encrypted_session<
            ...
            if (!ec) {
                ...
                spdlog::debug("Target host: {} {}", host_info_.ip,
                              host_info_.port);
            }
        });
}
...
client_tls_session::client_tls_session(
    ...
    socket_.set_verify_callback(
        std::bind(&client_tls_session::verify_certificate, this,
...
void client_session_base<SocketType>::request_file() {
    ...
    message_ = protocol::create_file_message(crypto_handler_.get_public_key(),
                                             j.dump());
...
void client_session_base<SocketType>::handle_read_file_request_response(
    ...
    if (!error) {
        ...
        j_resp["type"] = "accepted";

        auto wrapped = crypto_handler_.encrypt(pub_key_, j_resp.dump());

        if (!wrapped.has_value()) {
            spdlog::debug("decrypt failed");
        }

        j_resp = nlohmann::json(wrapped.value());
        message_ = protocol::wrap_with_header(j_resp.dump());
        ...
        async_write(
            socket_, boost::asio::buffer(message_.data(), message_.size()),
