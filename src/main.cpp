        ...
        while (temp_addr != nullptr) {
            if (temp_addr->ifa_addr->sa_family == AF_INET) {
                if (std::any_of(interface_names.begin(), interface_names.end(),
                                [&](const auto name) {
                                    return name.compare(temp_addr->ifa_name) ==
                                           0;
                                })) {
                    result.emplace_back(boost::asio::ip::make_address(inet_ntoa(
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (vm.count("help")) {
            if (vm.count("mode")) {
                usage_message = get_help_message(
                    misc::get_mode(vm["mode"].as<std::string>()));
            }
            print_help();
            return 0;
        }
        ...
        auto crypto_handler =
            std::make_unique<mfsync::crypto::crypto_handler>();
        crypto_handler->init("key.bin");
        ...
        if (vm.count("public_key")) {
            spdlog::set_pattern(
                std::string{mfsync::protocol::MFSYNC_LOG_PREFIX} + "%v");
        ...
        if (vm.count("trusted-keys")) {
            const auto& trusted =
                vm["trusted-keys"].as<std::vector<std::string>>();
        ...
        std::unique_ptr<mfsync::filetransfer::progress_handler>
            progress_handler =
        ...
        if (vm.count("outbound-interfaces")) {
            ...
            outbound_addresses.clear();
            outbound_addresses =
                get_ip_addresses_by_interface_name(address_vec);

            if (outbound_addresses.size() != address_vec.size()) {
                spdlog::info(
                    "Couldnt get addresses for all given outbound-interfaces.");
                spdlog::info(
                    "Multicast messages may not be sent to all interfaces.");
            }
        }
        ...
        if (vm.count("server-tls")) {
            const auto& file_paths =
                vm["server-tls"].as<std::vector<std::string>>();
            ...
            server_tls_paths =
                std::make_pair(file_paths.at(0), file_paths.at(1));
        }
        ...
        if (mode != operation_mode::FETCH && mode != operation_mode::GET) {
            for (const auto& outbound_address : outbound_addresses) {
                auto sender = std::make_unique<mfsync::multicast::file_sender>(
                    io_service, multicast_address, multicast_port, port,
                    file_handler, public_key);
                ...
                if (!outbound_address.is_unspecified()) {
                    if (outbound_address.is_v4()) {
                        ...
                        sender->set_outbound_interface(
                            outbound_address.to_v4());
                    } else {
                        spdlog::info(
                            "setting multicast outbound interface address to "
        ...
        if (mode != operation_mode::SHARE && mode != operation_mode::FETCH) {
            ...
            receiver = std::make_unique<mfsync::file_receive_handler>(
                io_service, file_handler, concurrent_downloads,
                *crypto_handler.get(), progress_handler.get());
